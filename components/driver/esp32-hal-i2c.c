...
#endif
    I2C_MUTEX_LOCK();
ESP_LOGI(TAG, "handle:%d",(int)i2c->intr_handle);
...
while(i<16){
  ...
  //ESP_LOGE(TAG,"[%2d] %c op[%d] val[%d] exp[%d] en[%d] bytes[%d]",i,(c.done?'Y':'N'),
  c.op_code,
  c.ack_val,
  c.ack_exp,
  c.ack_en,
...
*/
        ...
        if(i > 0) {
          ...
          //ESP_LOGE(TAG,"invalid CMD[] layout Stretch Failed");
          //dumpCmdQueue(i2c);
          done = true;
          }
        }
...
while(a<i2c->queueCount){
  tdq=&i2c->dq[a];
  ////ESP_LOGE(TAG,"[%d] %x %c %s buf@=%p, len=%d, pos=%d, eventH=%p bits=%x",a,tdq->ctrl.addr,(tdq->ctrl.mode)?'R':'W',(tdq->ctrl.stop)?"STOP":"",tdq->data,tdq->length,tdq->position,tdq->queueEvent,(tdq->queueEvent)?xEventGroupGetBits(tdq->queueEvent):0);
  uint16_t offset = 0;
...
////ESP_LOGE(TAG,"dev=%p date=%p level=%s",i2c->dev,i2c->dev->date,levelText);
#if !CONFIG_DISABLE_HAL_LOCKS
//ESP_LOGE(TAG,"lock=%p",i2c->lock);
#endif
//ESP_LOGE(TAG,"num=%d",i2c->num);
//ESP_LOGE(TAG,"mode=%d",i2c->mode);
//ESP_LOGE(TAG,"stage=%d",i2c->stage);
//ESP_LOGE(TAG,"error=%d",i2c->error);
//ESP_LOGE(TAG,"event=%p bits=%x",i2c->i2c_event,(i2c->i2c_event)?xEventGroupGetBits(i2c->i2c_event):0);
//ESP_LOGE(TAG,"intr_handle=%p",i2c->intr_handle);
////ESP_LOGE(TAG,"dq=%p",i2c->dq);
//ESP_LOGE(TAG,"queueCount=%d",i2c->queueCount);
//ESP_LOGE(TAG,"queuePos=%d",i2c->queuePos);
//ESP_LOGE(TAG,"byteCnt=%d",i2c->byteCnt);
...
// update Debug rxCount
  ...
  //ESP_LOGE(TAG,"RxEmpty(%d) call on TxBuffer? dq=%d",moveCnt,i2c->queuePos);
//  dumpI2c(i2c);
  }
...
if(p_i2c->stage==I2C_DONE){ //get Out
  ////ESP_LOGE(TAG,"eject int=%p, ena=%p",activeInt,p_i2c->dev->int_ena.val);
...
// need to complete this !  
  ...
  if(activeInt){ // clear unhandled if possible?  What about Disabling interrupt?
    ...
    //ESP_LOGE(TAG,"unknown int=%x",activeInt);
...
for(uint32_t a=1;a<=INTBUFFMAX;a++){
  ...
  if(intBuff[b][0][num]!=0) //ESP_LOGE(TAG,"[%02d] 0x%04x 0x%04x 0x%04x 0x%04x 0x%08x",b,((intBuff[b][0][num]>>16)&0xFFFF),(intBuff[b][0][num]&0xFFFF),((intBuff[b][1][num]>>16)&0xFFFF),(intBuff[b][1][num]&0xFFFF),intBuff[b][2][num]);
  }
#else
log_n("enable Core Debug Level \"Error\"");
#endif
...
if(i2c->i2c_event) {
  ...
  //ESP_LOGE(TAG,"eventCreate failed=%p",i2c->i2c_event);
  I2C_MUTEX_UNLOCK();
  return I2C_ERROR_MEMORY;
  }
...
if(!(eBits==EVENT_DONE)&&(eBits&~(EVENT_ERROR_NAK|EVENT_ERROR_DATA_NAK|EVENT_ERROR|EVENT_DONE))){ // not only Done, therefore error, exclude ADDR NAK, DATA_NAK
  ...
  //i2cDumpInts(i2c->num);
#else
  //ESP_LOGI("I2C exitCode=0x%x",eBits);
#endif
  }
...
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_ERROR
  //ESP_LOGE(TAG," Busy Timeout start=0x%x, end=0x%x, =%d, max=%d error=%d",tBefore,tAfter,(tAfter-tBefore),ticksTimeOut,i2c->error);
  //i2cDumpI2c(i2c);
  //i2cDumpInts(i2c->num);
#endif
    }
...
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_ERROR
  //ESP_LOGE(TAG," Gross Timeout Dead start=0x%x, end=0x%x, =%d, max=%d error=%d",tBefore,tAfter,(tAfter-tBefore),ticksTimeOut,i2c->error);
  //i2cDumpI2c(i2c);
  //i2cDumpInts(i2c->num);
#endif
    }
...
void i2cReleaseISR(i2c_t * i2c){
if(i2c->intr_handle){
  ESP_LOGI(TAG,"Release ISR %d",i2c->num);
  esp_err_t error =esp_intr_free(i2c->intr_handle);
  //if(error!=ESP_OK) //ESP_LOGE(TAG,"Error releasing ISR=%d",error);
