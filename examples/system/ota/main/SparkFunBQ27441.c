...
uint8_t _deviceAddress = BQ72441_I2C_ADDRESS;  // Stores the BQ27441-G1A's I2C address
	bool _sealFlag = false; // Global to identify that IC was previously sealed
	bool _userConfigControl = false; // Global to identify that user has control over
...
bool gaugeBegin(void)
{
	...
	ESP_LOGI(TAG, "wire begin");
	begin(5,18,100000); // Initialize I2C master
	
	deviceID = deviceType(); // Read deviceType from BQ27441

	if (deviceID == BQ27441_DEVICE_ID)
	{
		ESP_LOGI(TAG, "BQ success!");
...
bool setCapacity(uint16_t set_capacity)
{
	uint16_t current_cap = capacity((capacity_measure)DESIGN);

	if(set_capacity == current_cap)
		return true;
	// Write to STATE subclass (82) of BQ27441 extended memory.
	// Offset 0x0A (10)
	// Design capacity is a 2-byte piece of data - MSB first
	uint8_t capMSB = set_capacity >> 8;
	uint8_t capLSB = set_capacity & 0x00FF;
...
int16_t i2cReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	requestFrom(_deviceAddress, count, true);
